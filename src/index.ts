...
import { SCI__factory } from './services/contracts/typechain-types';
...
const SCI_CONTRACT_ADDRESS = '0xb015ac4d088b8693f18e16b820937875fb5703f6'

async function isWhitelistedAddressForDomain(domain: string, chainId: string, address: string): Promise<boolean> {
  const provider = getDefaultProvider('goerli')
  const SCIContract = SCI__factory.connect(SCI_CONTRACT_ADDRESS, provider)
  return SCIContract.isVerifiedForDomain(domain, chainId, address)
}
...
export const onTransaction: OnTransactionHandler = async ({
  ...
  const isWhitelisted = await isWhitelistedAddressForDomain(
